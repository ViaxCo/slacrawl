...
func TestMCPThreadWorkAcrossMessageBatches(t *testing.T) {
	...
	history := mcpHistoryStateForTest(t, st, "C123", "reference", "")
	require.True(t, history.Complete, "post-completion discovery ran before the concrete reply failure")
	require.Nil(t, history.Pending)
	pending, err := st.PendingThreadWork(ctx, SourceName, "TLOCAL", "C123")
	require.NoError(t, err)
