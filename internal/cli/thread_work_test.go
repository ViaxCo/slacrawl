...
func TestPendingThreadDoctorAndFreshness(t *testing.T) {
	for _, tc := range []struct {
		...
		{"api-only", "api-user", "bot", "partial", false},
		{"mcp-only", "mcp", "mcp", "full", false},
		{"api-with-success", "api-user", "bot", "partial", true},
		{"unrelated-source", "api-bot", "bot", "full", false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, tc.wantCoverage, report["slack_api"].(map[string]any)["thread_coverage"])
