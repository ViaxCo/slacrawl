...
- Persist retained-thread work during ordinary API sync, including when replies are unavailable. Resume it after lost history hints, failed replies, restarts or bot-to-user changes; keep explicit Since and Tail repair scoped. Retire work only after matching-generation success or committed deletion/purge cleanup; reconcile matching API thread skips for stored tombstones even without a pending job. Keep work out of Git share and freshness timestamps, and report pending API coverage as partial.
